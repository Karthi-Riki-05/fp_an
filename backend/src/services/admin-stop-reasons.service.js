  ...
  return withTenant(tenant, async (tx) => {
    const data = await tx.$queryRawUnsafe(
      `SELECT sr.id, sr.name, sr.type_id AS "typeId", sc.name AS "typeName",
       ...
       LEFT JOIN types sc ON sc.id = sr.type_id AND sc.entity = 'StopReason'::tenant_template."TypeEntity" AND sc.is_active = true AND sc.deleted_at IS NULL
       WHERE ${where}
    ...
    const totalRows = await tx.$queryRawUnsafe(
      `SELECT COUNT(*)::bigint AS count FROM stop_reasons sr WHERE ${where}`, ...params
    );
...
async function findOne(tenant, id) {
  ...
  if (!rows[0]) throw new NotFoundError('stop-reason-not-found');
  return rows[0];
}
