...
## Flow Monitor — pre-change state (2026-05-15)

The Flow Monitor at `/monitor/[[...id]]` (also re-exported at `/admin/monitor` since this session) currently renders the **same drawio iframe as the Designer** plus a heavy operator side-panel and KPI strip — neither of which match the legacy operator UX. Specifically: the iframe loads with `noSaveBtn=1` only — the rest of drawio's chrome (left shape library, top menu/toolbar, right Format panel, bottom status bar) is still visible, even though the editor is locked via `graph.setEnabled(false)`; a 7-column right card titled "Equipment #N" hosts the cascade-aware register-{production,scrap,stop} modals plus a hardcoded "Last 5 stops" list; a bottom Card carries hardcoded `KPIS = { oee: 78.4, availability: 92.1, performance: 88.6, quality: 96.0, partsOk: 1240, partsScrap: 18 }` mock values. The cell-click forwarder is already wired (`fp-embed.js` posts `event:'cellClick'` with `equipmentId`; `FlowDesignerEditor` exposes `onCellClick` and the Monitor page binds it to `setSelectedEquipmentId`). What's **missing** for the brief: the backend has **no POST endpoint** to create production/scrap/stop records — only GET (list) and PATCH (update) under both `/api/v1/results/*` and `/api/v1/admin/results/*` — and `admin-results.service.js` has no `createProduction/Scrap/Stop` functions, so the brief's described `POST /api/v1/admin/results/production` etc. would 404. The `/admin/results/upload-picture` endpoint for the camera-icon photo upload also doesn't exist (other upload routes do — `admin-flow-designs /background`, `admin-icons /upload`, `admin-machine-files /upload` — same multer + `FileStorageService` pattern). Equipment cascade endpoints all exist (`GET /equipment/:id/{parts,orders,stop-reasons,scrap-reasons,properties}`).
